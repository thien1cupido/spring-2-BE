...
public interface IProductRepository extends JpaRepository<Products, Integer> {
    ...
    @Query(value = "SELECT p.id AS id, p.name AS name, p.price AS price,p.quantity AS quantity, i.url AS image,p.descriptions AS description \n" +
            "FROM product AS p\n" +
    ...
    @Query(value = "SELECT p.id AS id, p.name AS name, p.price AS price,p.quantity AS quantity, i.url AS image ,p.descriptions AS description \n" +
            " FROM product AS p \n" +
            "        INNER JOIN images AS i on p.id = i.product_id\n" +
            " WHERE p.is_delete=false AND i.id IN (SELECT MIN(i.id) FROM images AS i GROUP BY i.product_id) ORDER BY p.create_time DESC ",
            countQuery =
                    " SELECT COUNT(*)" +
                            "            FROM product AS p" +
                            "                    INNER JOIN images AS i on p.id = i.product_id" +
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE product AS p SET p.is_delete=true WHERE p.id=:product_id", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE product AS p SET p.quantity=:quantity WHERE p.is_delete=false AND p.id=:product_id", nativeQuery = true)
    void updateQuantityById(@Param("quantity") Integer quantity, @Param("product_id") Integer id);
}
